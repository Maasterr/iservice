...
namespace iService
{
    class DataServiceMySql
    {
        public static List<iservice_users> CheckLogin(String login, String password)
        {
            using (MySqlConnection db = new MySqlConnection(ConfigurationManager.ConnectionStrings["Conn"].ConnectionString))
            {
        ...
        public static List<iservice_users> GetUsers()
        {
            using (MySqlConnection db = new MySqlConnection(ConfigurationManager.ConnectionStrings["Conn"].ConnectionString))
            {
                if (db.State == ConnectionState.Closed)
                    db.Open();
                return db.Query<iservice_users>("Select * from `dbo.iservice_users`").ToList();
            }
        }
        public static List<iservice_customers> GetAllCustomers()
        {
            using (MySqlConnection db = new MySqlConnection(ConfigurationManager.ConnectionStrings["Conn"].ConnectionString))
            {
                if (db.State == ConnectionState.Closed)
                    db.Open();
                return db.Query<iservice_customers>("Select * from `dbo.iservice_customers`").ToList();
            }
        }
        public static List<iservice_customers> CheckCustomer(String name,String surname,String patronymic)
        {
            using (MySqlConnection db = new MySqlConnection(ConfigurationManager.ConnectionStrings["Conn"].ConnectionString))
            {
                if (db.State == ConnectionState.Closed)
                    db.Open();
                return db.Query<iservice_customers>("Select * from `dbo.iservice_customers` where iservice_customers_name= '" + name+"'and iservice_customers_surname = '"+surname+"' and iservice_customers_patronymic='"+patronymic+"'").ToList();
            }
        }
        ...
        public static List<iservice_customers> GetCustomersById(int customers_id)
        {
            using (MySqlConnection db = new MySqlConnection(ConfigurationManager.ConnectionStrings["Conn"].ConnectionString))
            {
                if (db.State == ConnectionState.Closed)
                    db.Open();
                return db.Query<iservice_customers>("SELECT * FROM `dbo.iservice_customers` WHERE iservice_customers_id = " + customers_id).ToList();
            }
        }
        ...
        public static List<iservice_cars> GetCarsByCustomer(int id)
        {
            using (MySqlConnection db = new MySqlConnection(ConfigurationManager.ConnectionStrings["Conn"].ConnectionString))
            {
                if (db.State == ConnectionState.Closed)
                    db.Open();
                return db.Query<iservice_cars>("SELECT * FROM `dbo.iservice_cars` INNER JOIN `dbo.iservice_customers` ON `dbo.iservice_cars`.`iservice_cars_customers_id` = `dbo.iservice_customers`.`iservice_customers_id` WHERE(`dbo.iservice_cars`.`iservice_cars_customers_id` = " + id+")").ToList();
            }
        }
        ...
        public static List<iservice_cars> GetCarsByWord(String word)
        {
            using (MySqlConnection db = new MySqlConnection(ConfigurationManager.ConnectionStrings["Conn"].ConnectionString))
            {
                if (db.State == ConnectionState.Closed)
                    db.Open();
                return db.Query<iservice_cars>("SELECT * FROM `dbo.iservice_cars` WHERE(iservice_cars_reg_number LIKE '%' + N'" + word + "' + '%') OR  (iservice_cars_vin_number LIKE '%' + N'" + word + "' + '%')").ToList();
            }
        }
        ...
        public static List<iservice_cars> GetAllCars()
        {
            using (MySqlConnection db = new MySqlConnection(ConfigurationManager.ConnectionStrings["Conn"].ConnectionString))
            {
                if (db.State == ConnectionState.Closed)
                    db.Open();
                return db.Query<iservice_cars>("SELECT * FROM `dbo.iservice_cars`").ToList();
            }
        }
        ...
        public static List<iservice_orders_items> CheckOrderItem(int iservice_orders_item_id, int iservice_orders_items_orders_number)
        {
            using (MySqlConnection db = new MySqlConnection(ConfigurationManager.ConnectionStrings["Conn"].ConnectionString))
            {
                if (db.State == ConnectionState.Closed)
                    db.Open();
                return db.Query<iservice_orders_items>("SELECT * FROM `dbo.iservice_orders_items` where iservice_orders_items_orders_id = N'" + iservice_orders_items_orders_number + "' and iservice_orders_item_id=N'" + iservice_orders_item_id + "'").ToList();
            }
        }
        ...
        public static List<iservice_orders> GetAllOrders()
        {
            using (MySqlConnection db = new MySqlConnection(ConfigurationManager.ConnectionStrings["Conn"].ConnectionString))
            {
                if (db.State == ConnectionState.Closed)
                    db.Open();
                return db.Query<iservice_orders>("SELECT iservice_orders.*,iservice_orders_status.iservice_orders_status_name,iservice_orders_payment_status.iservice_orders_payment_status_name, iservice_users.iservice_users_name FROM `dbo.iservice_orders` LEFT JOIN iservice_orders_status ON `dbo.iservice_orders`.`iservice_orders_status_of_work` = `dbo.iservice_orders_status`.`iservice_orders_status_id` LEFT JOIN iservice_orders_payment_status ON iservice_orders.iservice_orders_status_of_payment = iservice_orders_payment_status.iservice_orders_payment_status_id LEFT JOIN iservice_users ON iservice_orders.iservice_orders_user_id = iservice_users.iservice_users_id").ToList();
            }
        }
        public static List<iservice_orders> GetOrdersByCar(int cars_id)
        {
            using (MySqlConnection db = new MySqlConnection(ConfigurationManager.ConnectionStrings["Conn"].ConnectionString))
            {
                if (db.State == ConnectionState.Closed)
                    db.Open();
                return db.Query<iservice_orders>("SELECT `dbo.iservice_orders`.*,`dbo.iservice_orders_status`.`iservice_orders_status_name`,`dbo.iservice_orders_payment_status`.`iservice_orders_payment_status_name`, `dbo.iservice_users`.`iservice_users_name` FROM `dbo.iservice_orders` LEFT JOIN `dbo.iservice_orders_status` ON `dbo.iservice_orders`.`iservice_orders_status_of_work` = `dbo.iservice_orders_status`.`iservice_orders_status_id` LEFT JOIN `dbo.iservice_orders_payment_status` ON `dbo.iservice_orders`.`iservice_orders_status_of_payment` = `dbo.iservice_orders_payment_status`.`iservice_orders_payment_status_id` LEFT JOIN `dbo.iservice_users` ON `dbo.iservice_orders`.`iservice_orders_user_id` = `dbo.iservice_users`.`iservice_users_id` where `dbo.iservice_orders`.`iservice_orders_cars_id` = " + cars_id ).ToList();
            }
        }
        public static List<iservice_orders> GetOrdersById(int order_id)
        {
            using (MySqlConnection db = new MySqlConnection(ConfigurationManager.ConnectionStrings["Conn"].ConnectionString))
            {
                if (db.State == ConnectionState.Closed)
                    db.Open();
                return db.Query<iservice_orders>("SELECT iservice_orders.*,iservice_orders_status.iservice_orders_status_name,iservice_orders_payment_status.iservice_orders_payment_status_name, iservice_users.iservice_users_name FROM `dbo.iservice_orders` LEFT JOIN `dbo.iservice_orders_status` ON `dbo.iservice_orders`.`iservice_orders_status_of_work` = `dbo.iservice_orders_status`.`iservice_orders_status_id` LEFT JOIN iservice_orders_payment_status ON iservice_orders.iservice_orders_status_of_payment = iservice_orders_payment_status.iservice_orders_payment_status_id LEFT JOIN iservice_users ON iservice_orders.iservice_orders_user_id = iservice_users.iservice_users_id where iservice_orders_id = " + order_id).ToList();
            }
        }
        /*public static List<iservice_orders> GetOrdersByCar(int cars_id)
        {
        ...
        public static List<iservice_orders_status> GetOrderStatusList()
        {
            using (MySqlConnection db = new MySqlConnection(ConfigurationManager.ConnectionStrings["Conn"].ConnectionString))
            {
                if (db.State == ConnectionState.Closed)
                    db.Open();
                return db.Query<iservice_orders_status>("SELECT * FROM `dbo.iservice_orders_status`").ToList();
            }
        }
        public static List<iservice_orders_payment_status> GetOrderPaymentStatusList()
        {
            using (MySqlConnection db = new MySqlConnection(ConfigurationManager.ConnectionStrings["Conn"].ConnectionString))
            {
                if (db.State == ConnectionState.Closed)
                    db.Open();
                return db.Query<iservice_orders_payment_status>("SELECT * FROM `dbo.iservice_orders_payment_status`").ToList();
            }
        }
        ...
        public static List<iservice_company> CompanyGetData(int iservice_company_key)
        {
            using (MySqlConnection db = new MySqlConnection(ConfigurationManager.ConnectionStrings["Conn"].ConnectionString))
            {
                ...
                return db.Query<iservice_company>("select * from `dbo.iservice_company` WHERE iservice_company_key = " + iservice_company_key).ToList();
            }
        }
